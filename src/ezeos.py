...
class GUI(QProcess):
    ...
    def __init__(self, parent=None):
        ...
        self.createEosioWalletButton = QPushButton("Create Eosio Wallet")
    ...
    def setPermissionObject(self):
        ...
        subprocess.check_output(
            [os.environ['CLEOS'], 'set', 'account', 'permission', self.account.name, 'active', multiSigPermissionObject,
             'owner', '-p', self.account.name + '@owner', ])
        ...
        out = subprocess.check_output(
            [os.environ['CLEOS'], 'set', 'account', 'permission', self.account.name, 'owner', multiSigPermissionObject,
    ...
    def setOwnerPermission(self):
        out = subprocess.check_output(
            [os.environ['CLEOS'], 'set', 'account', 'permission', self.account.name, self.account.creator,
             self.wallet.activePublicKey, '-p', self.account.name, '@', self.account.creator])
        self.getInfoLabel.setText(str(out))

    def stakeBandwidth(self):
        out = subprocess.check_output(
            [os.environ['CLEOS'], '--url', self.blockchain.producer, 'system', 'delegatebw', self.account.creator,
    ...
    def createEosioTokenAccount(self):
        ...
        try:
            ...
            self.importKeys()
            out = subprocess.check_output(
                [os.environ['CLEOS'], 'create', 'account', 'eosio', 'eosio.token', self.wallet.ownerPublicKey,
    ...
    def loadEosioContract(self):
        ...
        out = subprocess.check_output(
            [os.environ['CLEOS'], 'set', 'contract', 'eosio', os.environ['EOS_SOURCE'] + '/build/contracts/eosio.bios',
             '-p', 'eosio@active'])
        self.getInfoLabel.setText(str(out))
    ...
    def lockWallet(self):
        ...
        if self.wallet.locked == False:
            ...
            try:
                out = subprocess.check_output(
                    [os.environ['CLEOS'], 'wallet', 'unlock', '-n', self.wallet.name, '--password', word])
                word = ''
            except:
    ...
    def importKeys(self):
        ...
        if self.wallet.activePrivateKey == '':
            ...
            return
        try:
            subprocess.check_output([os.environ['CLEOS'], 'wallet', 'import', '-n', self.wallet.name, '--private-key',
    ...
    def createAccount(self):
        ...
        if self.blockchain.net == 'local':
            try:
                out = subprocess.check_output(
                    [os.environ['CLEOS'], 'create', 'account', 'eosio', self.account.name, self.wallet.ownerPublicKey,
                     self.wallet.activePublicKey, '-p', 'eosio'])
            except:
                out = 'Could not create account'
        elif self.blockchain.net == 'test':
            try:
                payload = {"create": {"creator": "eosio", "name": "sylvain", "owner": {"threshold": 1, "keys": [
                    {"key": self.wallet.ownerPublicKey, "weight": 1}], "accounts": [], "waits": []},
                                      "active": {"threshold": 1,
                                                 "keys": [{"key": self.wallet.activePublicKey, "weight": 1}],
                                                 "accounts": [], "waits": []}}}
                url = "http://{}/v1/chain/chain/abi_json_to_bin".format(self.blockchain.testProducer)
                #                 payload = {
                #   'code': 'eosio',
                #   'args': {
                #     'owner': {
                #       'keys': [
                #         {
                #           'key': self.wallet.ownerPublicKey,
                #           'weight': 1
                #         }
                #       ],
                #       'accounts': [],
                ...
                #       'keys': [
                #         {
                #           'key': self.wallet.activePublicKey,
                #           'weight': 1
                #         }
                #       ],
                #       'accounts': [],
                ...
                #print(payload)
                out = requests.request("POST", url, data=payload)
                print(out)
                # url = 'http://' + self.blockchain.testProducer
                # out = subprocess.check_output([os.environ['CLEOS'], '--url', url , 'create', 'account', 'eosio', self.account.name, self.wallet.ownerPublicKey, self.wallet.activePublicKey, '-p', 'eosio' ])
            except:
                out = 'Cannot create wallet'

        elif self.blockchain.net == 'main':
            permission = self.account.creator + '@active'
            out = subprocess.check_output(
                [os.environ['CLEOS'], '-u', self.blockchain.producer, 'system', 'newaccount', self.account.creator,
    ...
    def setContractSteps(self):
        ...
        try:
            if self.blockchain.net == 'local':
                out = subprocess.check_output(
                    [os.environ['CLEOS'], 'set', 'contract', self.account.name, self.order.contract, '-p',
                     self.account.name])
            elif self.blockchain.net == 'test' or self.blockchain.net == 'main':
                out = subprocess.check_output(
                    [os.environ['CLEOS'], '-u', self.blockchain.producer, 'set', 'contract', self.account.name,
                     self.order.contract, '-p', self.account.name])
    ...
    def issueCurrency(self):
        ...
        print(finalToken)
        out = subprocess.check_output(
            [os.environ['CLEOS'], 'push', 'action', self.account.name, 'create', finalToken, '-p',
    ...
    def buyRAM(self):
        ...
        out = subprocess.check_output(
            [os.environ['CLEOS'], '-u', self.blockchain.producer, 'system', 'buyram', self.account.name,
    ...
    def issueToAccount(self):
        ...
        out = subprocess.check_output(
            [os.environ['CLEOS'], 'push', 'action', self.account.name, 'issue', finalToken, '-p',
             self.account.name])  # + '@active'])
        self.getInfoLabel.setText(str(out))
    ...
    def transferToAccount(self):
        ...
        if self.blockchain.net == 'local':
            out = subprocess.check_output(
                [os.environ['CLEOS'], 'push', 'action', self.account.name, 'transfer', finalToken, '-p',
                 self.account.name])
        elif self.blockchain.net == 'test' or self.blockchain.net == 'main':
            out = subprocess.check_output(
                [os.environ['CLEOS'], '-u', self.blockchain.producer, 'push', 'action', self.account.name, 'transfer',
                 finalToken, '-p', self.account.name])
        self.getInfoLabel.setText(str(out))
    ...
    def sendToAccount(self):
        ...
        if self.blockchain.net == 'local':
            out = subprocess.check_output(
                [os.environ['CLEOS'], 'transfer', self.account.name, self.account.receiver, self.order.amount])
        elif self.blockchain.net == 'test' or self.blockchain.net == 'main':
            out = subprocess.check_output(
                [os.environ['CLEOS'], '-u', self.blockchain.producer, 'transfer', self.account.name,
                 self.account.receiver, self.order.amount])
        self.getInfoLabel.setText(str(out))
    ...
    def getAccountDetails(self):
        ...
        try:
            if self.blockchain.net == 'local':
                ...
                out = subprocess.check_output(
                    [os.environ['CLEOS'], '--url', self.blockchain.producer, 'get', 'account', self.account.name])
            self.getInfoLabel.setText(str(out))
        except:
    ...
    def getBalance(self):
        ...
        if self.blockchain.net == 'local':
            try:
                out = subprocess.check_output(
                    [os.environ['CLEOS'], 'get', 'currency', 'balance', 'eosio.token', self.account.name,
                     self.order.currency])
            except:
                print('')
        elif self.blockchain.net == 'main':
            out = subprocess.check_output(
                [os.environ['CLEOS'], '--url', self.blockchain.producer, 'get', 'currency', 'balance', 'eosio.token',
    ...
    def listWallets(self):
        ...
        try:
            ...
            print('Cannot list Wallet')
            out = 'Cannot list Wallet' + str(out)
        self.getInfoLabel.setText(str(out))
    ...
    def getBlockInfo(self):
        ...
        try:
            if self.blockchain.net == 'test':
                ...
                out = subprocess.check_output([os.environ['CLEOS'], '--url', self.blockchain.producer, 'get', 'block',
